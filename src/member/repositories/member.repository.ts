...
export class MemberRepository {
  ...
  async findMemberCountByConditions(
    memberDto: Partial<MemberEntity>,
  ): Promise<number> {
  ...
  async createMember(memberDto: Partial<MemberEntity>): Promise<number> {
    const insertResult: InsertResult = await this.memberRepository.insert(
      memberDto,
    );
  ...
  async updateMemberById(
    ...
    if (!affected) {
      throw new InternalServerErrorException(
        '회원 수정이 정상적으로 동작되지 않았습니다.',
      );
    }
  }

  async deleteMemberById(memberId: number): Promise<void> {
    const deleteResult: DeleteResult = await this.memberRepository.delete(
      memberId,
    );
